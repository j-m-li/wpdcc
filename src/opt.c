...
node *fold_reduce(node *n) {
	...
	if (OP_IFELSE == n->op && n->left && OP_GLUE == n->left->op) {
		m = n->left;
		if (m->left && m->right && m->left->left &&
			m->left->right && OP_BRFALSE == m->left->op)
		{
			m->left->left = fold_reduce(m->left->left);
			m->left->right = fold_reduce(m->left->right);
			m->right = fold_reduce(m->right);
		       	if (OP_LIT == m->left->left->op &&
				OP_LIT == m->left->right->op &&
				OP_LIT == m->right->op) 
			{
				if (m->left->left->args[0]) {
					n = mkleaf(OP_LIT, 
						m->left->right->args[0]);
				} else {
					n = mkleaf(OP_LIT, m->right->args[0]);
				}
			}
		}
	}
