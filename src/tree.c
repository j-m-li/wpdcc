...
static void emittree1(node *a) {
	...
	case OP_RSHIFT:	emittree1(a->left);
			emittree1(a->right);
			commit();
			ptr = !inttype(a->args[0]);
			if (PUNSIG == a->args[0] || 0 == a->args[0]) ptr = 1;
			if (!ptr) {
				ptr = !inttype(a->args[1]);
				if (PUNSIG == a->args[1]) ptr = 1;
			}
			if (a->op == OP_RSHIFT) {
				ptr ? genshru(1) : genshr(1); break;
			}
			switch(a->op) {
			case OP_DIV:	ptr ? gendivu(1) : gendiv(1); break;
			case OP_MOD:	ptr ? genmodu(1) : genmod(1); break;
			}
			break;
