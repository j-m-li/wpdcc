...
static void defmac(void) {
	...
	co = p; /* FIXME don't strip comments if in string literal */
	while (*co) {
		if (*co == '/') {
			co++;
			if (*co == '/') {
	...
	if ((y = findmac(name)) != 0) {
		if (strcmp(Stext[y], p)) /* FIXME: should be p not buf??? */
	...
	if (c == '"') {
		...
		if (pi[0]) {
			do {
				...
				strcat(path, file);
				pi = pi + strlen(pi) + 1;
			} while (((inc = fopen(path, "r")) == NULL) && pi[0]); 
		}
		if (inc) {
			fclose(inc);
		} else {
