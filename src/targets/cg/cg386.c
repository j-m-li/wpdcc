...
void cgdiv(void)	{ gen("cdq");
			  gen("idivl\t%ecx"); }
void cgdivu(void)	{ gen("xorl\t%edx,%edx");
			  gen("divl\t%ecx"); }
void cgmod(void)	{ cgdiv();
			  gen("movl\t%edx,%eax"); }
void cgmodu(void)	{ cgdivu();
			  gen("movl\t%edx,%eax"); }
void cgshl(void)	{ gen("shll\t%cl,%eax"); }
void cgshr(void)	{ gen("sarl\t%cl,%eax"); }
void cgshru(void)	{ gen("shrl\t%cl,%eax"); }
...
void cgautob(int l,int n)	{ ngen2("%s\t%%al,ConstL%d+%d(%%ebp)", "movb", l, n); }
void cgautow(int l, int n)	{ ngen2("%s\t%%eax,ConstL%d+%d(%%ebp)", "movl", l, n); }
...
void cgdefa(char *a, int v)	{ ngen(".long\t %s+%d", a, v); }
