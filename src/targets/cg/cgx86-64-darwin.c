...
void cgautob(int l,int n)	{ ngen2("%s\t%%al,ConstL%d+%d(%%rbp)", "movb", l, n); }
void cgautow(int l, int n)	{ ngen2("%s\t%%rax,ConstL%d+%d(%%rbp)", "movq", l, n); }
void cgdefa(char *a, int v)	{ ngen(".quad\t %s+%d", a, v); }
void cgdivu(void)	{ gen("xorq\t%rdx,%rdx");
			  gen("divq\t%rcx");
			}
...
void cgldgc(char *s)	{ sgen("%s\t%s,%%rax", "movsbq", s); }
void cgldlc(int n)	{ ngen("%s\t%d(%%rbp),%%rax", "movsbq", n); }
void cgldsc(int n)	{ lgen("%s\t%c%d,%%al", "movsbq", n); }
void cgshru(void)	{ gen("shrq\t%cl,%rax"); }
